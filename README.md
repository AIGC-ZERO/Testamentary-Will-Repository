...
> 青岛市遗嘱登记、见证、保管、执行、监管与纠纷调解一体化服务演示系统  
> **Testamentary Will Repository** · Vue 3 + Vite · 前端本地演示（公开仓库）
...
数据全部在浏览器本地运行（种子数据 + `localStorage`），无需后端服务，适合产品演示、交互评审与二次开发参考。
...
- **种子数据**：`src/seed.js` 提供登记、见证、纠纷、保管、加盟、短信、审计等演示样本。
...
- 未配置天地图密钥时仍可演示交互骨架；配置后加载真实青岛底图（见下文「天地图密钥」）。

### 适用场景

展厅投屏、领导驾驶舱、运维值班大屏、对外产品演示。可从后台侧栏「大屏展示」一键进入。
...
移动端优先：内容区最大宽度约 480px 居中全屏，无旁侧说明框，适合真机与窄屏演示。
...
```text
will-demo/
├── docs/images/          # README 配图（大屏 / 门户 / 后台 / H5）
├── public/               # 静态资源
├── src/
...
- 本仓库为**产品演示 / 交互原型**，业务规则与文案围绕「青岛岛城遗嘱库」场景设计，**不构成法律意见或正式业务系统**。
- 验证码、短信、人脸核验、支付等均为前端模拟，无真实第三方通道。
- 公开仓库中**不含**一键启停脚本、**不含**固定演示账号口令；请使用上文「快速开始」自行启动与登录。
...
演示项目，按需二次开发。贡献与 Issue 欢迎通过 GitHub 仓库提交。
